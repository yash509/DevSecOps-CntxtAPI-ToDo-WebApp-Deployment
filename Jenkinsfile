pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/cntxtapitodo"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-CntxtAPI-ToDo-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop cntxtapitodo | true"
                sh "docker rm cntxtapitodo | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name cntxtapitodo -p 3000:3000 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service cntxtapitodo-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"cntxtapitodo\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
